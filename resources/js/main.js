...
// If there is any text inside the item field(excluding only whitespaces so that blank activities cannot be created), add that text to the todo list
document.getElementById("add").addEventListener("click", function () {
  var value = document.getElementById("item").value;
  let check = 0;
  for (let i = 0; i < value.length; i++) {
    if (value.charAt(i) == " ") check++;
    else {
      check = 0;
      break;
    }
